...
namespace IOD {
    ...
    bool GLFW_SETUP(GLFWwindow* window) {
        ...
        glfwSetKeyCallback(window, [](GLFWwindow*, int key, int scancode, int action, int mods) {
            ...
            if (!glfw_to_key_code.contains(key)) {
                printf("[IOD] Pressed a glfw key and it is not mapped yet: %c\n", key);
                return;
            }

            KeyCode cb_code = glfw_to_key_code[key];
        ...
        glfwSetMouseButtonCallback(window, [](GLFWwindow* window, int button, int action, int mods) {
            ...
            KeyCode cb_code = glfw_to_key_code[button];
            UpdateInputCode(cb_code, action != GLFW_RELEASE);
            for (const auto profile : profiles) {
                if (!profile.active) {
                    continue;
